...
export class NewMigration1692199497672 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`Report\` ADD \`model\` varchar(255) NULL`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`User\` ADD \`refreshToken\` varchar(500) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`User\` ADD \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)`,
    );
    await queryRunner.query(
      `ALTER TABLE \`Report\` DROP FOREIGN KEY \`FK_abc8ce53e6ef1567f06400344fd\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`Report\` CHANGE \`userId\` \`userId\` int NOT NULL`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`User\` ADD UNIQUE INDEX \`IDX_4a257d2c9837248d70640b3e36\` (\`email\`)`,
    );
    await queryRunner.query(
      `ALTER TABLE \`User\` CHANGE \`createdAt\` \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`User\` CHANGE \`createdAt\` \`createdAt\` datetime NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`User\` DROP INDEX \`IDX_4a257d2c9837248d70640b3e36\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`User\` CHANGE \`createdAt\` \`createdAt\` datetime NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`User\` CHANGE \`isAdmin\` \`isAdmin\` tinyint NOT NULL DEFAULT '1'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`User\` CHANGE \`isAdmin\` \`isAdmin\` tinyint NOT NULL DEFAULT '1'`,
    );
    await queryRunner.query(
      `ALTER TABLE \`Report\` CHANGE \`userId\` \`userId\` int NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`Report\` ADD CONSTRAINT \`FK_abc8ce53e6ef1567f06400344fd\` FOREIGN KEY (\`userId\`) REFERENCES \`User\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
